...
const OrderScreen = () => {
  ...
  const handleDeliver = async () => {
    try {
      ...
      refetch();
      toast.success('Order As delivered');
    } catch (error) {
      toast.error(error?.data1?.message || error?.error);
    }
  };
                ...
                {userInfo &&
                  ...
                  !order.isDelivered && (
                    <button
                      ...
                      type='button'
                      className='rounded-md-border w-full border-transparent bg-indigo-600 px-5 py-3 text-base font-medium text-white shadow-sm transition-all hover:bg-indigo-700'>
                      Mark As delivered
                    </button>
                  )}
              </div>
